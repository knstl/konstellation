...
func networkWithAddressObjects(t *testing.T, n int) (*network.Network, []*types.Address) {
	...
	for i := 0; i < n; i++ {
		state.AddressList = append(state.AddressList, &types.Address{Creator: "ANY", Id: uint64(i)})
	}
...
func TestListAddress(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Address[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Address)
	})
}
