...
Example:
	...
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return generateNetwork(clientCtx, cmd, config, mbm, gus, genBalIterator, outputDir, chainID, minGasPrices,
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", kntypes.StakeDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01apple,0.001darc)")
...
func parseKeyStorage(keyStorageFile string) (*kntypes.KeyStorage, error) {
	var keyStorage kntypes.KeyStorage
...
func saveKey(nodeDir, keyringBackend, algoStr string, valKey *kntypes.ValidatorKey, key *kntypes.Key, inBuf io.Reader) error {
...
func parseNetConfig(netConfigFile string) (*kntypes.NetConfig, error) {
	var netConfig kntypes.NetConfig
...
func genAccounts(genaccs []*kntypes.GenAccount) ([]banktypes.Balance, []authtypes.GenesisAccount, error) {
	var (
	...
	for _, ga := range genaccs {
		...
		genBalances = append(genBalances, banktypes.Balance{
			Address: addr.String(),
			Coins: sdk.NewCoins(
				sdk.NewCoin(kntypes.StakeDenom, sdk.TokensFromConsensusPower(ga.CoinGenesis)),
			).Sort(),
		})
	}
...
func clientConfig(
	...
	validators []*kntypes.Validator) (err error) {
...
func configValidator(nodeConfig *tmconfig.Config,
	appConfig *srvconfig.Config,
	valInfo *kntypes.ValidatorInfo,
	key *kntypes.Key,
	genAccount *authtypes.GenesisAccount,
	keyringBackend,
	algoStr string,
	inBuf io.Reader,
) (*kntypes.Validator, error) {
	...
	valNodeConfig := kntypes.ValNodeConfig{
...
func configValidators(
	nodeConfig *tmconfig.Config,
	appConfig *srvconfig.Config,
	keyStorage *kntypes.KeyStorage,
	genAccounts []authtypes.GenesisAccount,
	netConfig *kntypes.NetConfig,
	keyringBackend,
	algoStr string,
	inBuf io.Reader,
) (validators []*kntypes.Validator, err error) {
...
func genTxs(
	...
	for _, validator := range validators {
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			...
			validator.ValPubKey,
			sdk.NewCoin(kntypes.StakeDenom, sdk.TokensFromConsensusPower(validator.Key.CoinDelegate)),
...
func generateNetwork(
	...
	if err := initGenFiles(clientCtx, mbm, gus, validators, genAccounts, genBalances, chainID); err != nil {
		return err
	}
